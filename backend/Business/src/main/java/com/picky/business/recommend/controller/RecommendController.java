...
public class RecommendController {
    ...
    public ResponseEntity<List<RecommendProductResponse>> getRecommendListByCombination(@RequestParam List<Long> productIdList) {
        return ResponseEntity.ok(recommendService.getRecommendListByCombination(productIdList));
    }
    ...
    public ResponseEntity<List<RecommendProductResponse>> getRecommendListByNutrient(@RequestParam List<Long> productIdList) {
        return ResponseEntity.ok(recommendService.getRecommendListByNutrient(productIdList));
    }

    @GetMapping ("/{category}")//카테고리 별 인기상품 추천
    public ResponseEntity<List<RecommendProductResponse>> getRecommendListByCategory(@PathVariable String category) {
        return ResponseEntity.ok(recommendService.getRecommendListByCategory(category));
    }
