...
# 최근 1시간동안의 검색 로그 가져오기
query = "(SELECT * FROM log_search WHERE created_at >= NOW() - INTERVAL 1 HOUR) as log_search"
df = spark.read \
    .jdbc(jdbc_url, query, properties=properties)
...
# 실시간 검색어 처리: 단어별로 카운트하고 상위 10개만 선택
search_keywords = df.groupBy("keyword").count().orderBy(desc("count")).limit(10)

# DataFrame을 리스트로 변환
keywords_list = [row['keyword'] for row in search_keywords.collect()]

# 리스트를 JSON 형식으로 변환 (ensure_ascii=False 추가)
search_json = json.dumps({"ranking": [{"keyword": k} for k in keywords_list]}, ensure_ascii=False)

# Redis 연결 설정
r = redis.Redis(host='j9a505a.p.ssafy.io', port=6382, db=0)  # Redis 호스트와 포트를 자신의 환경에 맞게 수정
